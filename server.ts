...
function deriveCategory(name: string, packaging: string, tagsStr: string): string {
    ...
    if (n.includes('ho') || n.includes('phổi') || n.includes('cold') || n.includes('siro ho') || n.includes('xoang') || n.includes('xịt mũi') || n.includes('mũi') || n.includes('họng') || n.includes('eugica') || n.includes('prospan')) {
        return 'Hô hấp & Cảm cúm';
    }
    if (n.includes('kháng sinh') || n.includes('cefixim') || n.includes('cefu') || n.includes('amox') || n.includes('augmentin') || n.includes('clari') || n.includes('tetra') || n.includes('azithro') || n.includes('klamentin') || n.includes('cipro')) {
        return 'Kháng sinh & Kháng khuẩn';
    }
    if (n.includes('paracetamol') || n.includes('panadol') || n.includes('hạ sốt') || n.includes('giảm đau') || n.includes('sủi') || n.includes('efferalgan') || n.includes('ibuprofen') || n.includes('meloxicam') || n.includes('diclofenac')) {
        return 'Giảm đau & Hạ sốt';
    }
    if (n.includes('tiêu hóa') || n.includes('men vi sinh') || n.includes('dạ dày') || n.includes('đại tràng') || n.includes('berberin') || n.includes('oresol') || n.includes('smecta') || n.includes('omepra') || n.includes('esomepra') || n.includes('ruột')) {
        return 'Tiêu hóa & Dạ dày';
    }
    if (n.includes('calci') || n.includes('calcium') || n.includes('vitamin') || n.includes('folic') || n.includes('sắt') || n.includes('kẽm') || n.includes('magne') || n.includes('khoáng chất') || n.includes('d3') || n.includes('dưỡng chất')) {
        return 'Vitamin & Khoáng chất';
    }
    if (n.includes('kem') || n.includes('gel') || n.includes('mụn') || n.includes('nghệ') || n.includes('nấm') || n.includes('liễu') || n.includes('bôi') || n.includes('mỡ') || n.includes('dị ứng') || n.includes('chàm')) {
        return 'Da liễu & Bôi ngoài da';
    }
    if (n.includes('tim mạch') || n.includes('huyết áp') || n.includes('amlodipin') || n.includes('statin') || n.includes('mỡ máu') || n.includes('xarelto') || n.includes('tiểu đường') || n.includes('đường huyết')) {
        return 'Tim mạch & Tiểu đường';
    }
    if (n.includes('bơm tiêm') || n.includes('bao cao su') || n.includes('băng keo') || n.includes('gạc') || n.includes('khẩu trang') || n.includes('nhiệt kế') || n.includes('muối') || n.includes('que thử') || n.includes('rửa mũi')) {
        return 'Thiết bị & Dụng cụ y tế';
    }
    if (n.includes('bổ') || n.includes('gan') || n.includes('thận') || n.includes('não') || n.includes('hoạt huyết') || n.includes('đông y') || n.includes('thảo dược') || n.includes('cao') || n.includes('trà') || n.includes('hoa sen')) {
        return 'Dược liệu & Bổ trợ sức khỏe';
    }
    return 'Dược phẩm thiết yếu';
}

// Generate realistic, deterministic price and stock based on product ID
const PRICE_TIERS = [
...
function generateStock(id: string): number {
    ...
    return (Math.abs(hash) % 95) + 12; // between 12 and 106 units
}
...
function processCSVData(csvText: string): Product[] {
    ...
    // Header: ['ID', 'Mã SP', 'Danh mục', 'Tên sản phẩm', 'Quy cách đóng gói', 'Tags', 'Link sản phẩm', 'Link ảnh']
...
// Load initial data from local file first, then fetch Google Sheets
async function loadInitialData() {
    try {
        const localCsvPath = path.join(process.cwd(), 'src', 'data', 'sheet_data.csv');
        if (fs.existsSync(localCsvPath)) {
            console.log('Loading initial data from local CSV snapshot...');
            const localCsv = fs.readFileSync(localCsvPath, 'utf-8');
            const loaded = processCSVData(localCsv);
            if (loaded.length > 0) {
                cachedProducts = loaded;
                lastSyncTime = new Date().toISOString();
                console.log(`Loaded ${loaded.length} medicines from initial snapshot!`);
            }
        }
    } catch (err) {
        console.error('Error loading initial CSV snapshot:', err);
    }

    // Attempt live sync from Google Sheets in background
    syncFromGoogleSheets().catch((err) => {
        console.warn('Initial Google Sheets live sync note:', err.message);
    });
}
...
async function syncFromGoogleSheets(): Promise<{ success: boolean; count: number; error?: string }> {
    ...
    try {
        console.log(`Fetching Google Sheet from ${GOOGLE_SHEET_CSV_URL}...`);
        ...
        lastSyncTime = new Date().toISOString();
        console.log(`Google Sheets live sync succeeded! ${products.length} products loaded.`);

        // Persist snapshot to disk for instant next startup
        try {
            const localCsvPath = path.join(process.cwd(), 'src', 'data', 'sheet_data.csv');
...
// 3. Paginated Products (Default 40 products per page as required)
app.get('/api/products', (req, res) => {
    const page = Math.max(1, parseInt(req.query.page as string) || 1);
    // Exactly 40 items per page by default
    ...
    if (search.trim()) {
        ...
        filtered = filtered.filter((p) => {
            return (
                ...
                p.tags.some((t) => normalizeVietnamese(t).includes(normQuery)) ||
...
async function startServer() {
    ...
    app.listen(PORT, '0.0.0.0', () => {
        ...
        console.log(`Connected to Google Sheets: ${GOOGLE_SHEET_ID} (GID ${GOOGLE_SHEET_GID})`);
    });
}
