...
export const OrderDetailModal: React.FC<OrderDetailModalProps> = ({
  ...
  const handleDelete = async () => {
    ...
    const confirmDelete = window.confirm(`Bạn có chắc chắn muốn xóa đơn hàng #${order.id} của khách hàng ${order.customerName}?`);
          ...
          <div className="p-4 bg-slate-50 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-200 rounded-xl transition-colors cursor-pointer"
              >
              ...
              {onDeleteOrder && (
                  <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-rose-600 hover:bg-rose-50 rounded-xl border border-rose-200 transition-colors cursor-pointer disabled:opacity-50"
                  >
                    <Trash2 className="w-3.5 h-3.5" />
                    <span>{isDeleting ? 'Đang xóa...' : 'Xóa đơn'}</span>
                  </button>
              )}
            </div>
            ...
            <div className="flex items-center gap-2">
              {onEditOrder && (
                  <button
                      ...
                      className="px-4 py-2.5 bg-amber-500 hover:bg-amber-600 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all shadow-xs cursor-pointer"
                  >
              ...
              <button
                  ...
                  className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 active:scale-98 text-white rounded-xl text-xs font-bold flex items-center gap-2 shadow-md shadow-blue-500/20 transition-all disabled:bg-slate-300 cursor-pointer"
              >
                {isUpdating ? (
                    <>
